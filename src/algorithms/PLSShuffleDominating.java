...
public class PLSShuffleDominating<S extends Solution> implements Algorithm<List<S>>, ZAlgorithm<S>, LocalSearchTester<S> {
	...
	public PLSShuffleDominating(Problem<S> problem, Supplier<List<List<Integer>>> indexGenerator, 
	...
	public String getName() {
		return "PLS-Shuffle-DominatingUpdate";
	}
	...
	public void run() {
		...
		while (explore.size() > 0) {
			...
			if (!candidate.attributes().containsKey("neighbourIndices")) {
				...
				if (shuffling) Collections.shuffle(neighbourIndices);
			...
			if (!candidate.attributes().containsKey("neighbourProgress")) 
				candidate.attributes().put("neighbourProgress", 0);
			int progress = (int) candidate.attributes().get("neighbourProgress");

			// neighbourhood exploration
			while (progress<neighbourIndices.size()) {
				S nb = neighbourGenerator.apply(candidate, neighbourIndices.get(progress));
				progress++;
				
				if (evaluations < maxEvaluations) {
					problem.evaluate(nb);
					evaluations++;
					int oldArchiveSize = archive.size();
					boolean added = archive.add(nb);
					if (added) explore.add(nb);
					if (monitor!=null) monitor.accept(this);
					
					// if dominating, update unstantly
					if (added && archive.size()<=oldArchiveSize) break;
				} else
					break;
			}
			
			candidate.attributes().put("neighbourProgress", progress);
			if (progress==neighbourIndices.size()) explore.remove(candidate);
