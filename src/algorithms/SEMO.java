...
public class SEMO<S extends Solution> implements Algorithm<List<S>>, ZAlgorithm<S>, LocalSearchTester<S> {
	...
	private S candidate;
	private int nonUpdateEvaluationSum;
	...
	public void run() {
		...
		while (archive.size() > 0) {
			...
			candidate = (S) archive.get(JMetalRandom.getInstance().nextInt(0, archive.size() - 1));
			...
			*/
			...
			if (added) nonUpdateEvaluationSum++;
			else nonUpdateEvaluationSum = 0;
			if (monitor!=null) monitor.accept(this);
			if (evaluations >= maxEvaluations || nonUpdateEvaluationSum >= stagnantStop) break;
		}
	}
