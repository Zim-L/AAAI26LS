...
public class PLSShuffleNonDominated<S extends Solution> implements Algorithm<List<S>>, ZAlgorithm<S>, LocalSearchTester<S> {
	...
	public PLSShuffleNonDominated(Problem<S> problem, Supplier<List<List<Integer>>> indexGenerator, 
	...
	public String getName() {
		return "PLS-Shuffle-NonDominatedUpdate";
	}
		...
		while (explore.size() > 0) {
			...
			while (progress<neighbourIndices.size()) {
				...
				if (evaluations < maxEvaluations) {
					...
					// if non-dominated or dominating, update unstantly
					if (added) break;
				} else
					break;
			}
